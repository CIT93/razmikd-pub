...
// Function 1
let dailyDesicions = function (fullTime) {
  let startTime = 8;

  if (startTime >= 8) {
    return `It is ${fullTime} a.m, time to start a new day!!`;
  }
};

let result = dailyDesicions(8);
showOnPage(result);

// Function 2 Email Check
let emailTime = function (checkEmail) {
  if (checkEmail <= 10) {
    return `It is ${checkEmail} a.m. Okay, time to check my emails. After that, I'll have my lunch break. `;
  }
};
